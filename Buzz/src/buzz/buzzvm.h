...
# define TIME_TO_FORGET_NEIGHBOUR 5

# define TIME_TO_ADAPT_NEIGHBOUR_CHANGE 10 

# define TIME_TO_FORGET_BLOB_REQUEST 1000
   /*
    * VM states
    */
   typedef enum {
   ...
   /*
    * A relocation chunk element for chunk monitor 
    */
   struct buzzchunk_reloc_elem_s
   {
     uint16_t id;
     uint16_t key;
     uint16_t cid;
     ...
     uint16_t time_to_destroy;
     buzzdarray_t checkednids;
   };
   typedef struct buzzchunk_reloc_elem_s* buzzchunk_reloc_elem_t;

   /*
    * A chunk monitor for chunk management 
    */
   struct buzzchunk_monitor_s
   {
     uint64_t chunknum;
     buzzdarray_t blobrequest; 
     buzzdarray_t getters;
     buzzdarray_t bidder; 
     uint8_t status;
   };
   typedef struct buzzchunk_monitor_s* buzzchunk_monitor_t;

    /*
    * A cmon chunk mangement struct 
    */
   struct buzzcmon_robot_specific_chunk_elem_s
   {
     uint16_t timetoforget; // time to for a neighbour to respond
     //buzzdarray_t chunks_on;
   };
   typedef struct buzzcmon_robot_specific_chunk_elem_s* buzzcmon_robot_specific_chunk_elem_t;

   /*
    * A destroy function for key chunks destroy 
    */
   extern void buzzvm_cmonitor_key_chunks_destroy(const void* key, void* data, void* params);

   extern int buzzvm_cmonitor_reloc_elem_key_cmp(const void* a, const void* b);
   /*
    * Buzz chunk monitor status type.
    * 
    */
   typedef enum {
      BUZZCMONITOR_FREE = 0,   // chunk monitor is free to accept blobs
      BUZZCMONITOR_RELOCATING  // chunk monitor relocating
   } buzz_cmon_status_e;
   
   /*
    * Buzz blob request monitor status type.
    * 
    */
   typedef enum {
      BUZZBLOB_REQUESTED_LOCATIONS = 0,       // requested locations
      BUZZBLOB_WAITING_TO_REQUEST_LOCATIONS,  // waiting to request locations
      BUZZBLOB_STATUS_CHANGE_DONE,            // status changed
      BUZZBLOB_WAITING_FOR_BLOB_AVILABILITY   // all chunks allocated not avilable waiting.
   } buzz_blobrequest_e;
   /*
    * VM data
    */
   struct buzzvm_s {
      ...
      /* Blob slot maps */
      buzzdict_t blobs;
      /* List of blob chunk stigmergy to refresh */
      buzzdarray_t chunk_stig;
      /* Neighbors active for chunk management */
      buzzdict_t active_neighbors;
      /* Time to adapt a neighbor change */
      uint16_t nchange;
      /* Chunk monitor for chunk management */
      buzzchunk_monitor_t cmonitor;
      ...
      uint32_t rngidx;
      /* Amount of out msg sent last step */
      uint64_t outmsgsstep;
      uint64_t p2poutmsgsstep;
      int receiver;
   };
   typedef struct buzzvm_s* buzzvm_t;

   /*
    * An active neighbour struct for chunk management 
    */
   struct buzzneighbour_chunk_s
   {
     uint16_t timetoforget; // time to forget a neighbour
     buzzdict_t chunks_on;
     //buzzdarray_t chunks_on;
   };
   typedef struct buzzneighbour_chunk_s* buzzneighbour_chunk_t;
   ...
   extern void buzzvm_neighbors_update(buzzvm_t vm);
   ...
   extern buzzdarray_t buzzvm_vm_serialize(buzzvm_t vm);

   extern void buzzvm_vm_deserialize_set(buzzvm_t vm, buzzdarray_t a);

   extern buzzdarray_t buzzvm_gsyms_serialize(buzzvm_t vm);

   extern int64_t buzzvm_gsyms_deserialize_set(buzzvm_t vm, buzzdarray_t a, int64_t pos);
 ...
 * @param vm The VM data.
 */
#define buzzvm_pusht(vm) { buzzobj_t o = buzzheap_newobj((vm), BUZZTYPE_TABLE); buzzvm_push(vm, o); }
